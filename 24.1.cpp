...
public:
    ...
    Client() {}
    Client(string n, int p, long rt) : name(n), priority(p), requestTime(rt) {}
};
...
public:
    ...
    PrintRecord() {}
    PrintRecord(const string& name, long time) : userName(name), printTime(time) {}
};

class PrintStatistics 
{
private:
    PrintRecord stats[MAX_STATS];
    int size;

public:
    PrintStatistics() : size(0) {}

    void recordPrint(const string& name, long printTime) 
    {
        if (size < MAX_STATS) 
        {
            stats[size] = PrintRecord(name, printTime);
            size++;
        }
    }
    ...
    void printStatistics() const 
    {
        ...
        {
            cout << "User: " << stats[i].userName << ", Time: " << stats[i].printTime << endl;
        }
    }
};
...
int main() 
{
    ...
    printerQueue.addClient(Client("Client1", 2, 10));
    printerQueue.addClient(Client("Client2", 1, 20));
    printerQueue.addClient(Client("Client3", 3, 30));

    while (!printerQueue.isEmpty()) 
    {
        Client nextClient = printerQueue.getNextClient();
        long printTime = nextClient.requestTime;

        cout << "Printing for user: " << nextClient.name << endl;

        printStats.recordPrint(nextClient.name, printTime);
    }
